...
async function main() {
  ...
  // Create SimpleAccountAPI instance
  ...
  // Get the counterfactual address (address before deployment)
  ...
  // Gas limits setup
  const res = await bundlerClient.estimateUserOpGas({  
    sender: SMART_ACCOUNT_ADDRESS,
    nonce: nonce,
    initCode: '0x', // actual bytes
    callData: counterCallData, // actual bytes
    paymasterAndData: '0x',
  });

  const callGasLimit = res.callGasLimit;
  const verificationGasLimit = res.verificationGas;
  const preVerificationGas = res.preVerificationGas;

  // Pack gas values into bytes32
  const accountGasLimits = packGasValues(BigNumber.from(verificationGasLimit), BigNumber.from(callGasLimit));
